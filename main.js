require('./settings')
const makeWASocket = require("@davetech/baileys").default
...
async function downloadSessionData() {
  try {
    ...
    if (!fs.existsSync(credsPath)) {
      ...
      const base64Data = global.SESSION_ID.split("davetech~")[1];
...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   store.bind(davetech.ev)

    // login use pairing code
   // source code https://github.com/WhiskeySockets/Baileys/blob/master/Example/example.ts#L61
        if (global.connect && !davetech.authState.creds.registered) {
        try {
			const custom = "TRASHBOT"; // must
            const phoneNumber = await question(chalk.cyan(`\n[ ᯤ ] davetech (--||--) Enter Your Number:\n`));
            const code = await davetech.requestPairingCode(phoneNumber.trim());
            console.log(chalk.green(`\n[ ᯤ ] davetech (--||--) Pairing Code:\n`), code);
        } catch (error) {
...
try{
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.badSession) {
				...
				startdavetech();
			} else davetech.end(`Unknown DisconnectReason: ${reason}|${connection}`)
		}
...
davetech.ev.on("messages.upsert",  () => { })
//------------------------------------------------------
...
davetech.ev.on('group-participants.update', async (anu) => {
    ...
    await welcome(iswel, isLeft, davetech, anu)
  })
    ...
    davetech.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            if (!davetech.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
    ...
    davetech.ev.on('contacts.update', update => {
        for (let contact of update) {
            let id = davetech.decodeJid(contact.id)
    ...
    davetech.getName = (jid, withoutContact = false) => {
        id = davetech.decodeJid(jid)
        withoutContact = davetech.withoutContact || withoutContact
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = davetech.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
                id,
                name: 'WhatsApp'
            } : id === davetech.decodeJid(davetech.user.id) ?
            davetech.user :
            (store.contacts[id] || {})
        return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }

davetech.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	let list = []
	for (let i of kon) {
	    list.push({
	    	displayName: await davetech.getName(i),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await davetech.getName(i)}\nFN:${await davetech.getName(i)}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Mobile\nEND:VCARD`
	    })
	}
	davetech.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }

    davetech.public = true

    davetech.serializeM = (m) => smsg(davetech, m, store)

    davetech.sendText = (jid, text, quoted = '', options) => davetech.sendMessage(jid, {
        text: text,
        ...options
    }, {
        quoted,
    ...
    davetech.sendImage = async (jid, path, caption = '', quoted = '', options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await davetech.sendMessage(jid, {
            image: buffer,
            caption: caption,
            ...options
        }, {
    ...
    davetech.sendTextWithMentions = async (jid, text, quoted, options = {}) => davetech.sendMessage(jid, {
        text: text,
    ...
    davetech.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
...
await davetech.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
.then( response => {
...
davetech.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
let buffer
if (options && (options.packname || options.author)) {
...
return buffer
    ...
    davetech.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
...
davetech.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    davetech.copyNForward = async (jid, message, forceForward = false, options = {}) => {
let vtype
...
await davetech.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
return waMessage
}
    
    davetech.sendPoll = (jid, name = '', values = [], selectableCount = 1) => { return davetech.sendMessage(jid, { poll: { name, values, selectableCount }}) }

davetech.parseMention = (text = '') => {
